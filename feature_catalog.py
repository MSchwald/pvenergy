...
class Feature:
    ...
    def display_name(self) -> str:
        """Name formating for printing tables and charts"""
...
class FeatureCatalog:
    ...
    PVDAQ_POA_IRRADIANCE = Feature("poa_irradiance_measured", Source.PVDAQ, unit="W/m²", required_features=(SYSTEM_ID,))
    ...
    UTC_OFFSET = Feature("utc_offset", Source.CALCULATED, data_type = int, is_constant = True, required_features=(TIME_ZONE,))
    LOCALIZED_TIME = Feature("localized_time", Source.CALCULATED, unit="h", data_type=pd.DatetimeIndex, required_features=(TIME, TIME_ZONE))
    ...
    PVLIB_POA_IRRADIANCE = Feature("poa_irradiance_calculated", Source.CALCULATED, unit="W/m²", required_features=(TILT, AZIMUTH, SOLAR_ZENITH, SOLAR_AZIMUTH, DNI, GHI, DHI), optional_features=(SURFACE_ALBEDO,))
    ...
    FAIMAN_U0 = Feature("faiman_u0", Source.CALCULATED, unit="W/m²K", is_constant = True, optional_features=(WIND_SPEED, PVLIB_POA_IRRADIANCE, TEMP_DIFFERENCE_MEASURED)) # General Heat Loss Coefficient ("Convective Heat Loss Coefficient at Zero Wind Speed")
    FAIMAN_U1 = Feature("faiman_u1", Source.CALCULATED, unit="W*s/m³K", is_constant = True, optional_features=(WIND_SPEED, PVLIB_POA_IRRADIANCE, TEMP_DIFFERENCE_MEASURED)) # Wind-Dependent Heat Loss Coefficient ("Wind Speed Coefficient")
    ...
    TIME_SINCE_SUNLIGHT = Feature("time_since_sunrise", Source.CALCULATED, unit="h", required_features=(TIME, DAY, PVLIB_POA_IRRADIANCE))
    ...
    POA_WIND_SPEED = Feature("poa*wind_speed", Source.CALCULATED, unit="W/sm", required_features = (PVLIB_POA_IRRADIANCE, WIND_SPEED))
    DCP_PER_AREA = Feature("dcp/area", Source.CALCULATED, unit="W/m²", required_features = (PVLIB_DC_POWER, AREA))
    ...
    GAMMA_POA = Feature("gamma*poa", Source.CALCULATED, unit="W/m²K", required_features = (GAMMA, PVLIB_POA_IRRADIANCE))
    RELATIVE_AZIMUTH = Feature("relative_azimuth", Source.CALCULATED, unit="°", required_features = (AZIMUTH, SOLAR_AZIMUTH))
