...
If you have docker installed, download `docker-compose.yaml` to an empty directory and run:
```bash
docker compose up
```
This script downloads the newest version of this project's docker image and trains models (if they are not already present in `data/models/`) on a sample set of features, then you can open [http://127.0.0.1:8000](http://127.0.0.1:8000) in your web browser to view the dashboard. You can also first run `docker compose up -d` and then `docker compose exec app pvenergy` together with any other commands of the underlying CLI (see the list below), for example
```bash
docker compose exec app pvenergy train --features <feature1 feature2 ...>
```
...
Moreover, provide `--tune` for using **Optuna** to optimize hyperparameters, the optional arguments `--trials <number=10>`, `--cv <number=3>` to specify the number of trials or cross-validations and `--dashboard` to view the live results on Optuna's dashboard ([http://localhost:8080](http://localhost:8080)).
...
* **`pipeline`**: Runs **`train`**, **`evaluate`** and then **`runserver`** to do everything in a row. If optional arguments `--features <feature1 feature2 ...>`, `--ids <id1 id2 ...>`, `--models <name1 ...>`, `--tune` etc. are provided, they get passed to the responsible commands.
...
* **Machine learning**: After cleaning up the training data, the pipeline `pvcore/ml` uses **scikit_learn**, **xgboost** and **lightgbm** for machine learning and **Optuna** for optimizing hyperparameters.
